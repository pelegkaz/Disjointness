...
From this point, the protocol follows the original proof. The players interpret the public randomness as a list of $(\ell, \alpha_i, \beta_i)$ triples where $\ell$ is a transcript and $\alpha_i, \beta_i \in [0,1]$.
Each player defines his set of candidates, transcripts that matches his estimated distribution for the other player.
In order to find a common candidate, the players agree on a list of hash functions, and Alice sends the random hashes of the elements to Bob. 
\end{proof}

\paragraph{Discussion.}
What is $D$? In this work we measure the discrepancy via
$D \coloneqq D_x + D_y = D(\tilde{\pi}_x\,\|\,\pi_x) + D(\tilde{\pi}_y\,\|\,\pi_y)$,
which compares the reference transcript marginals to the true ones. One might wonder whether $D(\mu\,\|\,\tilde{\mu})$ is suitable as well, but the answer is no.

\begin{example}[Why $D(\mu\,\|\,\tilde{\mu})$ is inadequate]
Consider distributions $\mu$ and $\tilde{\mu}$ over inputs where $X=Y$  (under both). 
In this case, each player knows exactly the other's input, therefore the distribution over its messages $q_x = p_y$. 
Although the distributions could be different, the conditional distributions $Y\mid X$ and $X\mid Y$ are constant in both distributions.
Therefore, every protocol can be easily compressed.
When observing the different divergences, we notice that the induced factors agree and $D = D_x + D_y = 0$ as expected.
