...
\begin{lemma}
    For a settings where there is a set $A \subseteq [N] $, $k$ players (which know $A$) and player $i$ holds a set $A_i$. Define $n = |A|$. If $|A_i| \leq \frac{|A|}{n^{1/k}}, A_i \subseteq A$. There is a deterministic protocol that reveals a set $B \subseteq A \setminus A_i$ and $|B| = \frac{n^{1/k}}{2}$ and costs $klog(n)$ bits of communication (using the coordinator settings). 
\end{lemma}
\begin{proof}
    Let us divide $A$ to groups sized $\frac{n^{1/k}}{2}$: $B_1, B_2, ...$. This is a division that every player knows since they all have $A, k$. Pay attention that there are more than $|A_i|$ groups since since $|A_i|(\frac{n^{1/k}}{2}) < |A_i|(n^{1/k}) \leq |A|$. Due to the pigeonhole principle, there is a group $B_j$ which is disjoint to $A_i$. \newline
    ...
    The only communication used is $j \in [|A_i|] \subseteq [n]$, therefore $klog(n)$. 
\end{proof}
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item 
      $|A_{i}| / {|A_{i-1}|} < 1 / n^{1/k}$
    \item $|A_{i-1}| \geq n^{1-\frac{i-1}{k}}$
  \end{enumerate}
  \label{lemma:narrow}
\end{lemma}
\begin{proof}
  ...
  Then we get that
  \begin{align*}
    \frac{|A_{j^\star - 1} \cap U_r|}{|A_0 \cap U_r|} = 
    \frac{|A_{j^\star - 1} \cap U_r|}{|A_{j^\star-2} \cap U_r|} \cdot \frac{|A_{j^\star-2} \cap U_r|}{|A_{j^\star-3} \cap U_r|} \cdot \ldots \cdot \frac{|A_1 \cap U_r|}{|A_0 \cap U_r|}
    \geq
    \left(  \frac{1}{n_r^{1f/k}} \right)^{j^\star}
    =
    \frac{1}{n_r^{\frac{j^\star - 1}{k}}}.
  \end{align*}
  Since $|A_0 \cap U_r| = n_r$, 
  \begin{align*}
      |A_{j^\star - 1} \cap U_r| \geq n_r^{1 - \frac{j^\star-1}{k}}
  \end{align*}
...
\begin{enumerate}[(1)]
  ...
  \item The participants use the protocol from lemma 2.1 in order to discover $Y \subseteq A_{i-1}^{(j)} \setminus X_i $ where $ |Y| > \frac{n^{1/k}}{2}$ 
  \item All participants set:
    \begin{itemize}
      \item $U \leftarrow U \setminus Y$,
      \item $n \leftarrow |U|$,
