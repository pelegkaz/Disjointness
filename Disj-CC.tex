...
All players maintain a \emph{global state}, which tracks the protocol's execution so far;
after $r$ iterations, the state will have the form $S_r = (c_1,j_1,B_1),\ldots,(c_r, j_r, B_r) \in \left( [k] \times \nat \times 2^{[n]} \right)^r$.
For convenience, let $S_0 = \lambda$, the empty string.

For a player $i$, an iteration $r$ and a state $s$,
let
\begin{equation*}
	\gamma_i^r(s) \coloneq \Pr_{(\rv{X}_1,\ldots,\rv{X}_k) \sim \mu}\left[ \text{$i$ is critical} \given \rv{X}_i = X_i, \rv{S}_{r} = s \right].
\end{equation*}
...
and sends the index $i$ to all players; everyone now sets $c_r = i$.

Next, using public randomness, the coordinator and the players sample
iid sets $A_{i-1}^1, \ldots A_{i-1}^2,\ldots \sim \mu(\rv{A}_{i-1}|\rv{S}_{r-1} = s_{r-1})$, where $s_{r-1}$
is the global state at the beginning of iteration $r$.
Player $i$ finds the first index $j$ such that $|A_{i-1}^j \cap X_i| / |A_{i-1}^j| < 1/n^{1/k}$
and sends it to the coordinator, along with the set $B = A_{i-1}^j \cap X_i$ to the coordinator.
The coordinator disseminates $j, B$ to all the other players.

Observe that $A_{i-1}^j \setminus B \subseteq \overline{X_i}$,
so the elements of $A_{i-1}^j \setminus B$ can now be removed from the universe:
\begin{equation*}
	U_r \leftarrow U_{r-1} \setminus (A_{i-1}^j \setminus B).
\end{equation*}
