...
\begin{lemma}
    For a settings where there is a set $A \subseteq [N] $, $k$ players (which know $A$), and player $i$ holds a set $A_i$ which $|A_i| \leq \frac{|A|}{n^{1/k}}, A_i \subseteq A$. There is a protocol that reveals $\frac{n^{1/k}}{2}$ zeros and costs $log(n)$. 
\end{lemma}
\begin{proof}
    ...
    The only communication used is $j \in [|A_i|] \subseteq [n]$, therefore $log(n)$. 
\end{proof}
