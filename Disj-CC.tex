...
We argue that the protocol may run only $O(n^{1-\frac{1}{k}})$ rounds. As described above, every round where $|U| = n$, we omit $\frac{n^{1-\frac{1}{k}}}{2}$ indexes. Therefore if $f(n)$ is the function of number of rounds when starting with universe sized $n$, we can define it by
    \begin{equation*}
        f(n) = \begin{cases}
               1               & n \leq N^{\frac{1}{k}}\\
           ...
           \end{cases}
    \end{equation*}
We use two simple technical claims in order to prove that $f(n) \in O(n^{1-\frac{1}{k}})$.
\begin{claim}
...
\begin{claim}
    ...
    \begin{equation*}
        f(n) = \begin{cases}
           ...
           \end{cases}
    \end{equation*}
    It is true that $f(n) \in O(n^{1-\frac{1}{k}})$.
\end{claim}
\begin{proof}
For the definition of $O$ notation, we need to prove that there exists a constant $D$ for which $\forall_{n, k \in \mathbb{N}} : f(n) \leq Dn^{1-\frac{1}{k}}$. Let us prove it by induction: \newline
