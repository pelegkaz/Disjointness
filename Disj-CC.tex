...
\end{lemma}
\begin{proof}
    Let us divide $A$ to groups sized $\frac{m}{2}$ where the last group may be sized up to $m -1$: $B_1, B_2, ...$. This is a division that every player knows even before communicating since they all have $A, k$. \newline
    ...
    The only communication used is $j \in [|A_i|]$, therefore $klog(n)$. 
\end{proof}
...
\begin{proof}
  ...
  \begin{equation*}
    A_{j-1} \cap U \neq \emptyset.
  \end{equation*}
  There is such a $j$, because $A_0 \cap U = U \neq \emptyset$.

  We claim that $A_{j} \cap U = \emptyset$.
  ...
  Thus,
  \begin{equation*}
    \frac{|A_j \cap U|}{|A_{j-1} \cap U|} = \frac{0}{|A_{j-1} \cap U|} = 0 < \frac{1}{n_r^{1/k}}
  \end{equation*}
  ...
  \begin{equation*}
    \frac{|A_i \cap U|}{|A_{i-1} \cap U|} \geq \frac{1}{n_r^{1/k}}.
  \end{equation*}
  Then we get that
  \begin{align*}
    \frac{|A_{j^\star - 1} \cap U|}{|A_0 \cap U|} = 
    \frac{|A_{j^\star - 1} \cap U|}{|A_{j^\star-2} \cap U|} \cdot \frac{|A_{j^\star-2} \cap U|}{|A_{j^\star-3} \cap U|} \cdot \ldots \cdot \frac{|A_1 \cap U|}{|A_0 \cap U|}
    \geq
  ...
  Since $|A_0 \cap U| = n_r$, 
  \begin{align*}
      |A_{j^\star - 1} \cap U| \geq n_r^{1 - \frac{j^\star-1}{k}}
  \end{align*}
    Therefore $A_{j^\star} \cap U \neq \emptyset$.
\end{proof}
...
\begin{enumerate}[(1)]
  ...
  \item Otherwise, the coordinator asks each player $i$ whether $i$ is significant (i.e., whether $\gamma_i(x_i, U) \geq \eps / kN^{1-\frac{1}{k}}$).
